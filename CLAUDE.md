...
- `public/` — 정적 파일.
- 이미지 업로드: **`@vercel/blob`** 사용. Server Action은 `src/app/admin/posts/new/_components/_image-upload/_services/upload-image.ts`. 업로드 경로는 `blog/{타임스탬프}-{파일명}`, 10MB 제한. Vercel 환경에서는 `BLOB_READ_WRITE_TOKEN`이 자동 주입되며, 로컬 개발 시 `.env.local`에 직접 설정 필요.
