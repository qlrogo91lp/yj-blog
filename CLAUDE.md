...
- **버튼 컴포넌트 분리**: 비즈니스 로직이 있는 버튼은 개별 컴포넌트 파일로 분리하고, 로직은 _actions로 뺀다. 레이아웃 컴포넌트(예: `bottom-bar.tsx`)는 이 버튼들을 조합만 한다.
- **Dialog 폴더 구조**: Dialog는 `_` prefix 폴더로 만들어 미니 페이지처럼 다룬다. 진입점은 `index.tsx`, 내부에 필요시 `_actions/`, `_components/`를 둔다.
  - 예: `_components/_preview/index.tsx`, `_components/_image-upload/index.tsx`
  - Dialog 전용 server action이나 client action은 해당 폴더의 `_actions/`에 둔다.
  - 단순한 dialog도 일관성을 위해 동일한 폴더 구조를 따른다.
