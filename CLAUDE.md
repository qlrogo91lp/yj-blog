...
- 각 라우트 경로에는 해당 페이지에서 사용하는 private 폴더를 사용한다. (_로 시작하는 폴더: _actions, _components, _hooks, ...등등)
- 각 페이지에 있는 _actions 폴더에는 컴포넌트에 기능이나 client 기능이 필요할때 최대한 작은 단위로 만들어서 사용한다. 예를 들어 submit-action.tsx에서는 zustand의 params를 읽어와서 server에 api 요청을 한다.
- 각 페이지에서 사용하는 zustand 상태값은 _store.ts 파일을 사용한다.
