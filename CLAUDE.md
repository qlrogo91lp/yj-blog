...
- `public/` — 정적 파일.
- 이미지 업로드: **Cloudflare R2** (`@aws-sdk/client-s3`) 사용. Server Action은 `src/app/admin/posts/new/_components/_image-upload/_services/upload-image.ts`. 업로드 경로는 `images/{타임스탬프}-{파일명}`, 10MB 제한. 필요한 환경 변수: `R2_ACCOUNT_ID`, `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY`, `R2_BUCKET_NAME`, `R2_PUBLIC_URL`.
