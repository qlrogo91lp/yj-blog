...
describe('postFormSchema', () => {
  ...
  describe('slug', () => {
    it('영소문자·숫자·하이픈 조합은 성공', () => {
      expect(
        postFormSchema.safeParse({ ...validData, slug: 'hello-world-123' })
          .success
      ).toBe(true);
    });
    ...
    it('대문자가 있으면 실패', () => {
      expect(
        postFormSchema.safeParse({ ...validData, slug: 'Test-Slug' }).success
      ).toBe(false);
    });

    it('공백이 있으면 실패', () => {
      expect(
        postFormSchema.safeParse({ ...validData, slug: 'hello world' }).success
      ).toBe(false);
    });

    it('빈 문자열이면 실패', () => {
      expect(postFormSchema.safeParse({ ...validData, slug: '' }).success).toBe(
        false
      );
    });
  });
  ...
  describe('status', () => {
    it('draft는 성공', () => {
      expect(
        postFormSchema.safeParse({ ...validData, status: 'draft' }).success
      ).toBe(true);
    });

    it('published는 성공', () => {
      expect(
        postFormSchema.safeParse({ ...validData, status: 'published' }).success
      ).toBe(true);
    });

    it('그 외 값은 실패', () => {
      expect(
        postFormSchema.safeParse({ ...validData, status: 'pending' }).success
      ).toBe(false);
    });
  });
