...
describe('PostContentAction', () => {
  ...
  describe('갤러리 내비게이션', () => {
    it('갤러리를 [data-gallery-wrap]으로 감싼다', () => {
      ...
      expect(
        container.querySelector('[data-gallery-wrap]')
      ).toBeInTheDocument();
    });
    ...
    it('이전/다음 사진 버튼을 생성한다', () => {
      ...
      expect(
        container.querySelector('button[aria-label="이전 사진"]')
      ).toBeInTheDocument();
      expect(
        container.querySelector('button[aria-label="다음 사진"]')
      ).toBeInTheDocument();
    });

    it('unmount 시 wrap과 버튼을 제거하고 갤러리를 원래 위치로 복원한다', () => {
      const { container, rerender } = render(
        <GalleryNavHandlerTestWrapper mounted={true} />
      );
      expect(
        container.querySelector('[data-gallery-wrap]')
      ).toBeInTheDocument();
