...
git log develop..HEAD --oneline    # develop 대비 커밋 목록
git status                         # 미커밋 변경사항 확인
gh pr list --head $(git branch --show-current)  # PR 존재 여부 확인
```

- 미커밋 변경사항이 있으면 **작업을 중단**하고 사용자에게 알린다.
- 현재 브랜치가 `main` 또는 `develop`이면 **작업을 중단**하고 사용자에게 알린다.

### 2. PR 생성 (없을 경우)

PR이 이미 존재하면 이 단계를 건너뛴다.

PR 타이틀과 바디는 `git log develop..HEAD --oneline` 결과를 기반으로 자동 구성한다.
...
git log develop --oneline -3      # develop에 커밋 반영 확인
```
