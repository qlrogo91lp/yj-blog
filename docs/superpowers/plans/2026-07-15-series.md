...
- [ ] **Step 2: schema.ts에 series 테이블 추가**
...
- [ ] **Step 3: cache-tags.ts에 series 태그 추가**
...
- [ ] **Step 1: 실패하는 테스트 작성** — `src/types/series.test.ts`
...
- [ ] **Step 4: index.ts export 추가** — `src/types/index.ts` 말미에 추가
...
- [ ] **Step 1: 구현** — `src/db/queries/series.ts`
...
- [ ] **Step 1: postFormSchema 테스트 갱신 (실패 확인용)** — `src/types/post.test.ts`
...
- [ ] **Step 3: postFormSchema에 seriesId 추가** — `src/types/post.ts`의 `categoryId` 라인 아래:
...
- [ ] **Step 5: 스토어에 seriesId 추가** — `src/app/admin/posts/new/_store.ts`
...
- [ ] **Step 6: save-post.ts에 seriesId 반영** — `src/app/admin/posts/new/_services/save-post.ts`
...
- [ ] **Step 7: 시리즈 셀렉터 작성** — `src/app/admin/posts/new/_actions/series-selector.action.tsx`
...
- [ ] **Step 9: 수정 페이지 초기화 반영** — `src/app/admin/posts/[id]/edit/_handlers/post-init.handler.tsx`의 `initializePost({...})` 호출에 `categoryId: post.categoryId,` 아래로 추가:
...
- [ ] **Step 5: page.tsx 연동** — `src/app/(main)/posts/[slug]/page.tsx`
...
- [ ] **Step 1: 실패하는 테스트 작성** — `src/app/(main)/series/_components/series-card.test.tsx`
...
- [ ] **Step 3: SeriesCard 구현** — `src/app/(main)/series/_components/series-card.tsx`
...
- [ ] **Step 5: 페이지 구현** — `src/app/(main)/series/page.tsx`
...
- [ ] **Step 1: 페이지 구현** — `src/app/(main)/series/[slug]/page.tsx`
...
- [ ] **Step 1: nav 테스트 갱신 (실패 확인용)** — `src/components/nav/nav-links.test.tsx`의 첫 테스트를 수정:
...
- [ ] **Step 2: 링크 추가** — `src/components/nav/nav-links.tsx`의 `links` 배열을 수정:
...
- [ ] **Step 3: sitemap에 시리즈 추가** — `src/app/sitemap.ts`
...
- [ ] **Step 1: E2E 테스트 작성** — `e2e/series.spec.ts`
