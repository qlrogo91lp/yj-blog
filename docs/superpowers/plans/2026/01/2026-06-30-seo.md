...
> ## ✅ 완료 (2026-07-01)
>
> 13개 Task 전부 `feature/seo` 브랜치에 구현 완료. subagent-driven-development로 Task별 implementer→리뷰 루프를 거쳐 진행. 전체 vitest 108개 통과, lint 0 error(사전 경고 3건은 본 브랜치 외), `npm run build` 성공. 최종 전체 브랜치 코드리뷰 수행.
>
> **결과 요약**
>
> - 입력 UI: `CharacterCounter`, `SeoSectionAction`(excerpt + AI 요약 버튼 + metaTitle), 이미지 alt popover
> - AI 요약: `generateExcerpt` Server Action(`claude-haiku-4-5`, `@anthropic-ai/sdk`)
> - 메타데이터: root layout `metadataBase`+기본 OG/Twitter, 글 상세 OG/Twitter/canonical, `BlogPosting` JSON-LD
> - 자산/문서: `public/og-default.png`(1200×630), `docs/seo-guide.md` 운영 가이드
> - 부수 수정: `submit-post`가 `excerpt`를 저장하지 않던 기존 버그 수정, 기존 image-toolbar 테스트 3개 라벨 정정
>
> **최종 리뷰 후속 조치 (해소됨)**: 사이트 URL 환경변수가 코드베이스에서 `NEXT_PUBLIC_SITE_URL`(prod fallback `https://yjlogs.com`)과 `NEXT_PUBLIC_BASE_URL`(localhost fallback) 두 갈래로 혼재하던 사전 불일치를 발견 → `NEXT_PUBLIC_SITE_URL` 하나로 일원화(sitemap/robots/layout/글상세). 프로덕션 절대 URL의 localhost fallback SEO 결함 제거. 더불어 미발행 글의 `generateMetadata`를 `status==='published'`로 가드.
>
> **운영자 수행 항목으로 남김**: Task 13의 수동 UI 검증(dev 서버 + 페이지 소스 보기)·외부 검증(Rich Results Test / 카카오·슬랙 SNS 카드 / Search Console + sitemap 제출).
...
- [x] **Step 1: `type State`에 필드 2개 추가**
...
- [x] **Step 2: `type Action`에 setter 2개 추가**
...
- [x] **Step 3: `initializePost` 인자 타입에 `metaTitle` 추가**
...
- [x] **Step 1: `SavePostInput`에 `metaTitle` 추가**
...
- [x] **Step 2: `parsed.data` destructure에 `metaTitle` 추가**
...
- [x] **Step 3: UPDATE payload에 필드 추가**
...
- [x] **Step 4: INSERT payload에 필드 추가**
...
- [x] **Step 5: `submit-post.ts`에 store 필드 전달**
...
- [x] **Step 6: `post-init.handler.tsx`에 `metaTitle` 초기화 추가**
...
- [x] **Step 3: `ImageToolbar`에 alt popover 추가**
...
- [x] **Step 5: `ImageNodeView`에서 alt 바인딩**
...
- [x] **Step 1: 1200×630 OG 기본 이미지 준비**
...
- [x] **Step 5: `ArticleJsonLd` 컴포넌트 작성**
...
2. **이 블로그가 출력하는 메타데이터 지도** — 표로: 각 메타가 _누구에게/어디서_ 보이는지
...
- [x] **Step 2: 작성 흐름 수동 확인**

Run: `npm run dev`

- [x] `/admin/posts/new` → "SEO 설정" 토글 보임 → 클릭 시 펼쳐짐
- [x] 본문 입력 후 "AI로 요약 생성" 클릭 → excerpt 채워짐 (`ANTHROPIC_API_KEY` 설정 시) → 직접 수정 가능
- [x] 본문 비어있으면 AI 버튼 비활성
- [x] metaTitle 60자에서 카운터 yellow, 100자에서 destructive
- [x] 이미지 클릭 → 툴바 톱니 클릭 → popover로 alt input → 입력 후 외부 클릭 시 닫힘·값 유지
- [x] 글 저장 후 수정 페이지 재진입 시 excerpt/metaTitle 값 복원

- [x] **Step 3: 발행 후 메타 검증**

발행한 글 상세에서 "페이지 소스 보기":

- [x] `<title>`, `<meta name="description">`, `<link rel="canonical">`
- [x] `og:type=article`, `og:title/description/image/url`, `article:published_time`, `article:tag`
- [x] `twitter:card=summary_large_image`, `twitter:title/description/image`
- [x] `<script type="application/ld+json">` 안 `BlogPosting`
- [x] 썸네일 없는 글 → OG 이미지 `og-default.png` fallback

- [x] **Step 4: 외부 검증 (선택, 배포 후)**

- [x] Rich Results Test로 BlogPosting 인식·오류 0
- [x] 카카오톡/슬랙에 URL 붙여 카드 미리보기 정상
