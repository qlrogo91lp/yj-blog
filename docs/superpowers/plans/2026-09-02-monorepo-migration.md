...
> **완료: 2026-09-02.** Task 1~5를 subagent-driven-development로 실행해 `main`까지 머지하고 프로덕션 배포를 확인했다(`yjlogs.com` 200). 관련 PR: #89(develop), #91(phantom dependency 수정), #90(main). 결과 요약은 아래 "실행 결과" 참조.

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

## 실행 결과 (2026-09-02)

**최종 구조:** 루트에 `package.json`(workspace + turbo 스크립트), `pnpm-workspace.yaml`, `turbo.json`, `.nvmrc`, `.prettierignore`, `README.md`. 앱은 `apps/web` 하나. 히스토리는 `git mv`로 전부 보존됐다(438개 파일 100% rename).

**계획과 달랐던 점**

- **숨은 의존성이 3개 나왔다.** `@tiptap/core`(런타임), `@types/hast`·`@types/mdast`(타입 전용). npm의 flat `node_modules`에서 우연히 접근되던 phantom dependency로, pnpm strict 링킹에서 전부 깨졌다. 앞의 하나는 Task 2에서, 뒤의 둘은 Vercel preview 빌드에서 드러났다.
- **로컬 빌드 검증이 오염돼 있었다.** 작업 worktree(`.claude/worktrees/`)가 옛 npm `node_modules`를 가진 메인 체크아웃 **내부에** 중첩돼 있어, TypeScript가 상위로 올라가 그쪽 `@types`를 찾아 통과시켰다. 타입 전용 phantom dependency 둘이 로컬에서 안 잡힌 이유다. 다음에 같은 작업을 한다면 워크트리를 저장소 바깥에 두거나, 검증 전에 상위 `node_modules`를 정리해야 한다.
- **`pnpm install`에 `allowBuilds` 승인이 필요했다.** pnpm 11이 postinstall을 기본 차단해 `ERR_PNPM_IGNORED_BUILDS`가 났다. `@clerk/shared`·`esbuild`·`msw`·`sharp`·`unrs-resolver` 5개를 `pnpm-workspace.yaml`에 승인했다.
- **`eslint-plugin-react-hooks`를 7.0.1로 고정했다.** lockfile 재생성 시 `^7.0.0` 범위 안에서 7.1.1로 드리프트하며 새 규칙(`set-state-in-effect`)이 에러를 냈다. src 미수정 원칙에 따라 버전 고정으로 처리했다. 훅 로직 수정은 별도 과제로 남았다.
- **전체 Prettier 정리를 되돌렸다.** Task 2 Step 9 지시대로 `pnpm format`을 돌렸더니 `docs/` 아래 기존 문서 57개까지 바꿔서, Global Constraint("docs/ 아래 기존 문서는 수정하지 않는다")와 충돌했다. 원복하고 `.prettierignore`에 `docs/`를 추가했다.
- **`turbo.json`에 `env`를 추가했다.** `envMode: "loose"`는 환경변수를 태스크에 넘기지만 캐시 해시에는 넣지 않아, 값만 바꾸고 빌드하면 옛 값이 박힌 `.next`가 캐시로 재생된다. `.env.example`의 13개 변수를 `build`·`test:run`에 선언했다.
- **Task 5 Step 2(첫 preview 실패 확인)는 성립하지 않았다.** 이 프로젝트의 Vercel Ignored Build Step이 `Only build production` 프리셋이라 preview 배포가 전부 취소되고 있었다. 검증을 위해 일시적으로 `Automatic`으로 바꿔 develop → main PR에서 preview 빌드를 돌렸고, 거기서 위의 타입 phantom dependency를 잡았다.

**Vercel에서 확인된 사실**

- `packageManager: pnpm@11.22.0`을 존중한다. 로그: `Detected pnpm-lock.yaml version 9 generated by pnpm@10.x with package.json#packageManager pnpm@11.22.0` → `Done in 28s using pnpm v11.22.0`
- `allowBuilds`가 반영돼 `sharp` postinstall이 정상 실행된다.
- `apps/web/vercel.json`의 `ignoreCommand`가 대시보드 Ignored Build Step 설정보다 우선한다.
- **`turbo-ignore`는 deprecated다.** 빌드 로그 경고: `"turbo-ignore" is deprecated. Use Vercel's built-in project skipping instead.` Root Directory 설정의 "Skip deployments when there are no changes to the root directory or its dependencies" 토글로 대체하는 것이 권장된다. → 후속 과제

**남은 후속 과제**

- `turbo-ignore` → Vercel 네이티브 project skipping으로 교체, `vercel.json` 정리
- Vercel Ignored Build Step을 `Only build production`으로 되돌리기(검증용으로 `Automatic`으로 바꿔둔 상태)
- 메인 체크아웃의 옛 npm `node_modules` 정리 후 루트에서 `pnpm install`
- `.claude/rules/testing.md`의 Playwright webServer 설명이 아직 `npm run dev` (플랜이 `.claude/rules/*.md` 수정을 금지해 손대지 않음)
- `useInfinitePosts.ts`의 `react-hooks/set-state-in-effect` 대응 후 `eslint-plugin-react-hooks` 핀 해제
- `apps/web/playwright.config.ts`의 `webServer.url`이 특정 랜딩 라우트(`/apps/ralli`)를 가리킴 — 기존 이슈
...
- [x] Vercel yjlogs 프로젝트 Root Directory를 `apps/web`으로 변경
- [x] 변경 후 preview 재배포 성공
- [x] develop 머지 후 같은 날 main 머지
