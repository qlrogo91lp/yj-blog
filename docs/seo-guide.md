...
5. **네이버 서치어드바이저 등록 + sitemap 제출 (최초 1회)** — [서치어드바이저](https://searchadvisor.naver.com)에 사이트를 등록(소유 확인)하고, `요청 > sitemap 제출` 메뉴에서 `sitemap.xml`을 제출합니다. 네이버는 구글과 별도의 검색로봇(Yeti)을 쓰므로 구글 Search Console에 등록했다고 네이버 검색에도 자동으로 노출되지는 않습니다. `RSS 제출`(`/feed.xml`)도 함께 등록해두면 새 글이 더 빨리 수집됩니다.
   > 참고: 네이버는 OpenGraph를 부분적으로만 읽고 JSON-LD `BlogPosting`은 거의 참고하지 않습니다. 검색 결과에는 `title`/`meta description` 문구가 그대로 노출되는 비중이 커서, excerpt·SEO 제목을 쓸 때 검색될 키워드를 자연스럽게 포함시키는 것이 구글보다 더 중요합니다.
6. **`ANTHROPIC_API_KEY` 환경변수 확인** — "AI로 요약 생성" 버튼은 Claude API(`generateExcerpt`, `src/app/admin/posts/new/_services/generate-excerpt.ts`)를 호출합니다. 환경변수 `ANTHROPIC_API_KEY`가 설정되어 있지 않으면 버튼을 눌렀을 때 "ANTHROPIC_API_KEY가 설정되지 않았습니다" 에러가 발생합니다. 로컬·배포 환경 모두 `.env`에 키를 등록해야 합니다.
